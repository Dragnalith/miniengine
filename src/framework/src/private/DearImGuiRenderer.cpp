...
struct DearImGuiRendererImpl {
    DearImGuiRendererImpl(RenderDevice& device) : renderDevice(device) {}
    RenderDevice& renderDevice;
    ID3D12DescriptorHeap*        descriptorHeap = nullptr;
};

DearImGuiRenderer::DearImGuiRenderer(RenderDevice& device, DearImGuiManager&)
    : m_impl(device)
{
    ...
    ImGui_ImplDX12_Init(m_impl->renderDevice.GetDevice(), NUM_FRAMES_IN_FLIGHT,
        DXGI_FORMAT_R8G8B8A8_UNORM, m_impl->descriptorHeap,
        m_impl->descriptorHeap->GetCPUDescriptorHandleForHeapStart(),
        m_impl->descriptorHeap->GetGPUDescriptorHandleForHeapStart());
